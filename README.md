...
- **Parallelism.** `--workers/-j` (default `0` = auto from CPU count, capped at 8; `1` = sequential)
  fans the independent per-page OCR and per-value matching out over a thread pool. It only changes
  *how fast* a run finishes, never the result: work is reassembled in input order and every unit is
  a pure function of its input, so output is byte-for-byte identical to `-j 1`.
